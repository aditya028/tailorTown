...
export default function Card({ tailor }) {
  ...
  const handleClick = () => {
    const dataStr = JSON.stringify(tailor);
    navigate(`/tailor/${encodeURIComponent(dataStr)}`);
  };

  return (
    <div
      onClick={handleClick}
      className="flex flex-col p-2  border-[#696969] rounded-lg cursor-pointer hover:shadow-round hover:shadow-[#696969]"
    >
